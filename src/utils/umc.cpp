...
int main(int argc, char *argv[])
{
	...
	while( !exit ) {
		...
		//load uri type payload for deferring command scheme
		else if(args[0] == "loadCache" ) {
			printf("command :  %s\n",cmd.c_str());
			mp.state = LOADED;
			...
			string property = "setpropertytest";
			mp.selectTrack(trackType, 1);
			mp.play();

			string uri = "settesturi";
			mp.setUri(uri);
			uri = "testSubtitleuri";
			mp.setSubtitleSource(uri);
			mp.setSubtitleEnable(1);
			mp.setSubtitlePosition(4);
			mp.setSubtitleSync(1500);
			string key = "bufferRangeInterval";
			mp.setUpdateInterval(key,300);
			mp.setProperty(property);
			mp.setDescriptiveVideoService(1);
		}
		else if( args[0] == "play") {
			printf("command :  %s\n",cmd.c_str());
			mp.play();
			...
			if( args.size() != 2 ) {
				printf("setUri - usage :  setUri uri \n");
				continue;
			}
		...
		else if(args[0] == "setSubtitleSource" ) {
			...
			if( args.size() != 2 ) {
				printf("setSubtitleSource - usage :  setSubtitleSource uri \n");
				continue;
			}

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleSource(args[1]);
		}
		else if(args[0] == "setSubtitleEnable" ) {
			...
			if( args.size() != 2 ) {
				printf("setSubtitleEnable - usage :  setSubtitleEnable enable \n");
				continue;
			}
			bool display_enable = boost::lexical_cast<int>(args[1]);
			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleEnable(display_enable);
		}
		else if(args[0] == "setSubtitlePosition" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitlePosition - usage :  setSubtitlePosition position \n");
				continue;
			}

			string type = args[1];
			long position =  boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitlePosition(position);
		}
		else if(args[0] == "setSubtitleSync" ) {
			if( UNLOADED ==  mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleSync - usage :  setSubtitleSync sync \n");
				continue;
			}

			long sync =  boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleSync(sync);
		}
		else if(args[0] == "setSubtitleFontSize" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleFontSize - usage :  setSubtitleFont fontsize \n");
				continue;
			}

			long fontSize =  boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleFontSize(fontSize);
		}
		else if(args[0] == "setSubtitleColor" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleColor - usage :  setSubtitleSync color \n");
				continue;
			}

			long color =  boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleColor(color);
		}
		else if(args[0] == "setSubtitlePresentationMode" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitlePresentationMode - usage :  setSubtitlePresentationMode presentationMode \n");
				continue;
			}

			std::string presentationMode = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitlePresentationMode(presentationMode);
		}
		else if(args[0] == "setSubtitleCharacterColor" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleCharacterColor - usage :  setSubtitleCharacterColor charColor \n");
				continue;
			}

			std::string charColor = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleCharacterColor(charColor);
		}
		else if(args[0] == "setSubtitleCharacterOpacity" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleCharacterOpacity - usage :  setSubtitleCharacterOpacity charOpacity \n");
				continue;
			}

			long charOpacity = boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleCharacterOpacity(charOpacity);
		}
		else if(args[0] == "setSubtitleCharacterFontSize" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleCharacterFontSize - usage :  setSubtitleCharacterFontSize charFontSize \n");
				continue;
			}

			std::string charFontSize = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleCharacterFontSize(charFontSize);
		}
		else if(args[0] == "setSubtitleCharacterFont" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleCharacterFont - usage :  setSubtitleCharacterFont charFont \n");
				continue;
			}

			std::string charFont = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleCharacterFont(charFont);
		}
		else if(args[0] == "setSubtitleBackgroundColor" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleBackgroundColor - usage :  setSubtitleBackgroundColor bgColor \n");
				continue;
			}

			std::string bgColor = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleBackgroundColor(bgColor);
		}
		else if(args[0] == "setSubtitleBackgroundOpacity" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleBackgroundOpacity - usage :  setSubtitleBackgroundOpacity bgOpacity \n");
				continue;
			}

			long bgOpacity = boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleBackgroundOpacity(bgOpacity);
		}
		else if(args[0] == "setSubtitleCharacterEdge" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleCharacterEdge - usage :  setSubtitleCharacterEdge charEdgeType \n");
				continue;
			}

			std::string charEdgeType = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleCharacterEdge(charEdgeType);
		}
		else if(args[0] == "setSubtitleWindowColor" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleWindowColor - usage :  setSubtitleWindowColor windowColor \n");
				continue;
			}

			std::string windowColor = args[1];

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleWindowColor(windowColor);
		}
		else if(args[0] == "setSubtitleWindowOpacity" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 2 ) {
				printf("setSubtitleWindowOpacity - usage :  setSubtitleWindowOpacity windowOpacity \n");
				continue;
			}

			long windowOpacity = boost::lexical_cast<int>(args[1]);

			printf("command :  %s\n",cmd.c_str());
			mp.setSubtitleWindowOpacity(windowOpacity);
		}
		else if(args[0] == "setUpdateInterval" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 3 ) {
				printf("setUpdateInterval - usage :  setUpdateInterval key value \n");
				continue;
			}

			int32_t currentTimeInterval = boost::lexical_cast<int>(args[1]);
			int32_t bufferRangeInterval = boost::lexical_cast<int>(args[2]);

			printf("command :  %s\n",cmd.c_str());
			mp.setUpdateInterval(currentTimeInterval, bufferRangeInterval);
		}
		else if(args[0] == "setUpdateIntervalKV" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			long value =  boost::lexical_cast<int>(args[2]);

			printf("command :  %s\n",cmd.c_str());
			mp.setUpdateInterval(key, value);
		}
		else if(args[0] == "takeSnapshot" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 6 ) {
				printf("takeSnapshot - usage :  takeSnapshot location format width height pictureQuality \n");
				continue;
			}
			...
			printf("command :  %s\n",cmd.c_str());
			mp.takeSnapshot(args[1],args[2],width,height,pq);
		}
		else if(args[0] == "startRecord" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 3 ) {
				printf("startRecord - usage :  startRecord location format \n");
				continue;
			}

			printf("command :  %s\n",cmd.c_str());
			mp.startRecord(args[1],args[2]);
		}
		...
		else if(args[0] == "changeResolution" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 3 ) {
				printf("changeResolution - usage :  changeResolution width height \n");
				continue;
			}

			long width =  boost::lexical_cast<int>(args[1]);
			long height =  boost::lexical_cast<int>(args[2]);

			printf("command :  %s\n",cmd.c_str());
			mp.changeResolution(width,height);
		}
		else if(args[0] == "setStreamQuality" ) {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 5 ) {
				printf("setStreamQuality - usage :  changeResolution width height \n");
				continue;
			}

			long width =  boost::lexical_cast<int>(args[1]);
			long height =  boost::lexical_cast<int>(args[2]);
			long bitRate =  boost::lexical_cast<int>(args[3]);
			bool init =  boost::lexical_cast<int>(args[4]);

			printf("command :  %s\n",cmd.c_str());
			mp.setStreamQuality(width,height,bitRate,init);
		}
		else if(args[0] == "setProperty" ) {
			...
			if( args.size() != 2 ) {
				printf("setProperty - usage :  setProperty payload \n");
				continue;
			}

			printf("command :  %s\n",cmd.c_str());
			mp.setProperty(args[1]);
		}
		else if(args[0] == "setDescriptiveVideoService" ) {
			...
			if( args.size() != 2 ) {
				printf("setDescriptiveVideoService - usage : setDescriptiveVideoService enable(disable) \n");
				continue;
			}
			bool dvs_enable = boost::lexical_cast<int>(args[1]);
			printf("command :  %s\n",cmd.c_str());
			mp.setDescriptiveVideoService(dvs_enable);
		}
			...
			if( args.size() != 3 ) {
				printf("setMasterClock - usage : setMasterClock ip port \n");
				continue;
			}
			string ip = args[1];
			int32_t port = boost::lexical_cast<int>(args[2]);
			printf("command :  %s\n",cmd.c_str());
			mp.setMasterClock(ip, port);
		}
		else if (args[0] == "setSlaveClock") {
			if( UNLOADED == mp.state ) {
				printf("load content first !\n");
				continue;
			}
			...
			if( args.size() != 4 ) {
				printf("setSlaveClock - usage : setSlaveClock ip port baseTime \n");
				continue;
			}
			string ip = args[1];
			int32_t port = boost::lexical_cast<int>(args[2]);
			int64_t baseTime = boost::lexical_cast<int>(args[2]);
			printf("command :  %s\n",cmd.c_str());
			mp.setSlaveClock(ip, port, baseTime);
		}
		...
		else if(args[0] == "logPipelineState" ) {
		...
		else if(args[0] == "setPipelineDebugState" ) {
			if( args.size() != 2 ) {
				printf("setPipelineDebugState - usage :  setPipelineDebugState "
						" <json_debug_state>\n");
				continue;
			}

			printf("command :  %s\n",cmd.c_str());
			mp.setPipelineDebugState(args[1]);
		}
